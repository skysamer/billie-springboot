...
## 7. 회고록 및 아쉬운 점
소스를 작성하면서 제 코드가 단일책임원칙을 상당부분 위배한다는 것을 깨달았습니다. 가령 법인카드 부분에서 하나의 서비스단이 법인카드와 관련된 모든 api 요청을 처리하고있었는데 사실 지금 생각해보면 법인카드 반납, 법인카드, 법인카드 예약이라는 3개의 큰 줄기로 나눠 각각의 책임에 따라 클래스를 분리할 수 있었다고 생각합니다.
  
또한 테스트코드를 제대로 작성하지 않은 점도 아쉽습니다. 단순히 포스트맨으로 동작여부와 예외상황을 점검하면 된다고 생각했습니다. 하지만 리팩토링할때 테스트코드의 중요성이 부각되었던 것 같습니다. 
